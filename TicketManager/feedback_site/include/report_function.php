...
function GetReportOutput($project_id, $report_id, $action)
{
	...
	if (($action == "new") || ($action == "update")) {
		...
		$summary = '<textarea class="input-form-text-field" name="summary" maxlength="200">'.$summary.' </textarea>';
		...
		$all_version_sql = "select * from proj_version_table where project_id='".$project_id."' order by version_name desc";
		$root_version_result = $GLOBALS['connection']->Execute($all_version_sql) or DBError(__FILE__.":".__LINE__);
		$root_version_line = $root_version_result->Recordcount();
		// �p�G���]�w Area �� Minor Area ���ܡA�N�C�X�U�Ԧ����H�ѿ��
		// �p�G�S���h��X��r����H�ѿ�J
		if($root_version_line != 0) {
			$return_string = '<select size="1" name="version" onChange="change()"><option></option>';
			while ($root_version_row = $root_version_result->FetchRow()){
				$version_name = $root_version_row["version_name"];
				$return_string.= '<option>'.$version_name.'</option>';
			}
			$return_string.= '</select>';
		}else{
			if ($version_pattern == "") {
				$return_string.= '<input class="input-form-text-field" type="text" name="version" value="'.$_GET['version'].'" maxlength="40">';
			} else {
	...
	$return_string = '
	<table align="center" border="1" style="border-collapse: collapse" cellpadding="2" cellspacing="0">
		<tr>
