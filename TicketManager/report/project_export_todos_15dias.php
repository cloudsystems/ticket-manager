...
<input type="hidden" name="report_time" value="<?php echo $_GET['report_time']?>">
...
<form method="GET" name="back_index_form" action="/index.php">
<input type="hidden" name="report_time" value="<?php echo $_GET['report_time']?>">
<input type="hidden" name="search_key" value="<?php echo stripslashes($_GET['search_key'])?>">
<input type="hidden" name="search_type" value="<?php echo $_GET['search_type']?>">
<input type="hidden" name="page" value="<?php echo stripslashes($_GET['page'])?>">
...
<div id="current_location">
	...
	if (isset($project_id) && $report_time>0){
    
     	echo '<a href="JavaScript:DocumentChange(document.back_form);" style="cursor: pointer;"><?php echo $project_name?></a> /';
	
    }else{
		echo '<a href="JavaScript:DocumentChange(document.back_index_form);" style="cursor: pointer;"><?php echo $project_name?></a> /';
    }
	
	if (isset($report_time) && $report_time>0){
		$title="Exportar incidencias creadas los últimos $report_time días";
    } else{
		$title="Exportar total de incidencias ";
...
<div id="main_container">
		...
		<tr>
			<td width="100%" align="left" nowrap>
				<img src="<?php echo $GLOBALS["SYS_URL_ROOT"]?>/images/outline_export.png" width="48" height="48" align="middle" border="0">
					<tt class="outline"><?php echo $title;?></tt>
			</td>
	...
	if (isset($project_id) && $report_time>0){
     	...
     	echo '<a href="JavaScript:DocumentChange(document.back_form);" style="cursor: pointer;"><img src="'.$GLOBALS["SYS_URL_ROOT"].'/images/return.png" border="0" align="middle">'.$STRING['back'].'</a>';
	
    }else{
		echo '<a href="JavaScript:DocumentChange(document.back_index_form);" style="cursor: pointer;"><img src="'.$GLOBALS["SYS_URL_ROOT"].'/images/return.png" border="0" align="middle">'.$STRING['back'].'</a>';
    }
    ?>		
	...
	<div id="sub_container" style="width:90%;">
		...
		<div class="aw-box-ml"><div class="aw-box-mr"><div class="aw-box-mc">
			...
			<form method="POST" name="main_form" action="project_doexport_ali_15.php">
				<input type="hidden" name="project_id" value="<?php echo $_GET['project_id']?>">
				<input type="hidden" name="report_time" value="<?php echo $_GET['report_time']?>">
				<input type="hidden" name="search_key" value="<?php echo stripslashes($_GET['search_key'])?>">
				<input type="hidden" name="search_type" value="<?php echo $_GET['search_type']?>">
...
?>
				...
				<p align="center"><input type="button" value="<?php echo "Exportar Incidencias"?>" name="B1" class="button" onClick="CheckIconv(document.main_form);"></p>
			</form>
		</div></div></div>
