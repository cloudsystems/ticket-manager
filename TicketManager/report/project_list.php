 ...
 *
 * Version:	$Id: project_list.php,v 1.42 2009/07/06 07:04:04 alex Exp $
 *
 */
...
if ($_SESSION[SESSION_PREFIX.'uid'] != 0) {
	...
	if ($line != 1) {
		...
		$lastread = $check_auth_result->UserTimeStamp($check_auth_result->fields["last_read"], "Y-m-d H:i:s");
...
if ( isset($_GET['search_key']) && (eregi("^#+[0-9]+[0-9]*$", $_GET['search_key']))) {
...
on (puat.project_id = pat.project_id and puat.area_id=pat.area_id and puat.user_id='".$_SESSION[SESSION_PREFIX.'uid']."' and puat.project_id='".$_GET['project_id']."')";
...
if (strlen($condition)>0 && strlen($condition_area)>0){
	$condition = $condition."and (".$condition_area.")";
}elseif (strlen($condition_area)>0){
	$condition = "where ".$condition_area;
}
...
if ($_GET['project_id'] != 1)
{	

echo '
			<td nowrap valign="bottom">
				<a href="report_new.php?project_id='.$_GET['project_id'].'">
					<img src="'.$GLOBALS["SYS_URL_ROOT"].'/images/new_ticket.jpg" align="middle" border="0">&nbsp;
					'.$STRING['new_report'].'
				</a>&nbsp;
			</td>';
}
...
echo '<script>
 		...
 		<div id="export_image" >
		<a href="#" onclick="showExport()">
		 <img src="../images/export.png" border="0" align="middle" >Exportar a Excel</img>
		</a>
        </div>
 		...
 		<div id="export" align="right" style="display:none"> 
 		
 		<script>
		function loadurl()
		{
			window.location=document.exportform.export.options
                    [document.exportform.export.selectedIndex].value
        }
		</script>
		<form name="exportform" >
		 <font color="#42649B"><?php echo "Informes:"?></font>
		    <select name="export" size="1" >
		      <option selected value="/report/project_export_todos_15dias.php?project_id='.$project_id.'&report_time=15">Ultimos 15 dias (xls)</option>
			  <option value="/report/project_export_todos_15dias.php?project_id='.$project_id.'&report_time=30">Ultimos 30 dias (xls)</option>
			  <option value="/report/project_export_todos_15dias.php?project_id='.$project_id.'">Todos (xls)</option>         
		      
		      <option value="/report/project_export_ali.php?project_id='.$project_id.'">Todas (xml)</option>
     		    </select>
                    
		   <a href="JavaScript:loadurl()"><input type="button" style="display:inline-block" value="Exportar" border="0" img src="../images/export.png"  border="0" ></a>        
		</form>  
		</div>
...
<!--			 <td nowrap valign="bottom">
                                <a href="JavaScript:ChangeURL('project_export_todos_15dias.php', -1, false);">
                                <img src="<?php echo $GLOBALS["SYS_URL_ROOT"]?>/images/export.png" border="0" align="middle"><?php echo "Ultimos 15 dias"?></a>
                        </td>

		 <td nowrap valign="bottom">
                                <a href="JavaScript:ChangeURL('project_export_todos.php', -1, false);">
                                <img src="<?php echo $GLOBALS["SYS_URL_ROOT"]?>/images/export.png" border="0" align="middle"><?php echo "Todos Proyectos"?></a>
                        </td>

			<td nowrap valign="bottom">
				<a href="JavaScript:ChangeURL('project_export.php', -1, false);">
				<img src="<?php echo $GLOBALS["SYS_URL_ROOT"]?>/images/export.png" border="0" align="middle"><?php echo "Historico"?></a>
			</td>  --> 
...
?>
			...
			<td class="title" align="center" nowrap><?php echo $STRING['function']?></td>
...
for ($i = 0; $i < sizeof($show_column_array); $i++) {
	...
	if ($setting_row[$show_column] == 't') {
		...
		case "verified_by":
			echo '
				<select class="select_header" name="'.$show_column_array[$i].'" onChange="document.main_form.'.$show_column_array[$i].'.value=this.options[this.selectedIndex].value; DocumentReload();">
...
if ($_GET['project_id'] != 1)
{
	...
	echo '
			...
			echo '<td width="10" align="center" class="'.$td_class.'">&nbsp;</td>
		</tr>';
